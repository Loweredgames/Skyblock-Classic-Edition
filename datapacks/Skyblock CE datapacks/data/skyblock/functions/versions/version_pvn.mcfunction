...
##VERSION PVN FOOLS (LASCIARE SEMPRE COSI)
function skyblock:versions/setup_version
function skyblock:versions/pvn_remove


##PVN VERSION PROTOCOL FOOLS (AGGIORNARE SEMPRE NELLE NUOVE VERSIONI. UTILIZZARE UN NUMERO INTERO PER LE VERSIONI STABILI. UTTILIZARE UNA LETTERA PER LE VERSIONI DI SVILUPPO)
scoreboard objectives add PVN_aprilfools_2024 dummy
scoreboard players add @a PVN_aprilfools_2024 1
execute as @a[scores={SkyblockCE_id=10}] run scoreboard objectives remove PVN_aprilfools_2024


##PVN VERSION LEGACY PROTOCOL FOOLS (DA AGGIUNGERE SOLO QUANDO IL NUMERO CAMBIA)
scoreboard objectives add PVN_legacy dummy
execute as @a[scores={PVN_aprilfools_2024=1..}] run scoreboard players add @a PVN_legacy 1
